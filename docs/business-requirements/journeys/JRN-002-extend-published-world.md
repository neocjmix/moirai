...
- 해당 World의 Publication과 공개본이 존재한다.
...
5. LLM은 해당 Canon의 기존 사실을 존중하면서 새 사건·관계·서술을 구성하고 작성 유래를 남긴다한다.
6. 작성자는 변경될 부분과 유지될 부분을 검토한다.
7. 승인된 변경은 비공개 관리 상태에 반영되며 기존 공개본은 그대로 유지된다.
8. 출판자는 새 내용을 공개할지, 공개 범위에 포함할지 결정한다.
9. Atropos는 갱신된 공개본을 공개한다.
10. 기존 독자는 이전에 사용하던 공개 참조를 통해 적절한 맥락으로 계속 접근할 수 있다.

## JRN-002.5 성공 결과

- 선택한 Canon의 기존 사실과 새 내용이 중복 없이 연결된다.
- 공개 전까지 기존 공개본은 영향을 받지 않는다.
