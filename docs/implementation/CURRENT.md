...
현재 배포는 v4 Canon 모델이다. main `1b837bf7457090da2d4225219fa72265865ec8bf`와 Railway 세 서비스 latest SUCCESS가 일치한다. 역사 World Live export는 revision30이다. IP-004~010 완료 이력은 유지한다.
...
A2는 [owner-full DB inventory와 암호화 backup/restore 도구](../evidence/ip011/a2-execution.md)부터 진행 중이다. 운영 schema/content migration은 아직 실행하지 않았다. 그다음 v5+데이터 rehearsal → A3 cutover → A4 bounded read → A5 discovery → A6 역사 dogfooding이다.
