...
- 설계 검토와 사용자 승인을 받은 문서는 `accepted`로 변경한다. 구현 달성 여부는 CURRENT와 evidence에서 별도 추적한다.
...
| TS-005 | [World 근거의 파생 모델과 선택 projection](TS-005-derived-models.md)    | Subject·Process·State·Duration·Timeline                                     | accepted |
| TS-006 | [Atropos 공개 읽기와 그래프 탐색](TS-006-atropos-publication.md)        | Snapshot, URL, 검색, URDR-derived SVG graph                                 | accepted |
...
- 파생 모델은 근거와 algorithm version을 가지며 World의 새 사실을 만들지 않는다.
- Atropos는 React·Next.js와 URDR-derived SVG를 사용하고 Revision별 공개 artifact를 읽는다.
...
TS-010의 temporal relation 계약은 배포 v4에 반영되어 있다. IP-011의 Collection/단일 Narrative/World-level assertion target은 아직 미구현이며 CURRENT와 IP-011이 이행 상태를 소유한다.
...
| 그래프 표현 | URDR-derived SVG와 semantic zoom을 사용하고 복합 영역은 시간축 sweep envelope로 계산한다. 잠재 3D 확장은 TS-009에서 별도 검증한다. | TS-006, TS-009         |
