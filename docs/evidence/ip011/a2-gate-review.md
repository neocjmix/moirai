...
- Deploy and run the read-only `scripts/ip011-a2-scenario-rehearsal.ts rehearse-scenarios` on the explicitly named clone. Its six fixed Event IDs, broader title queries, shared membership, contains/Collection overlap and installed-World count yield an actual-data matrix without exposing prose; inspect failures before counting any gate. The installation has one World, so distinct-World behavior needs synthetic integration evidence, not a false actual-data claim.
