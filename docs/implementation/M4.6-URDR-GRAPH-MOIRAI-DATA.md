...
- pointer drag, wheel, pinch-centroid zoom, selection, detail sheet와 URL 복원
...
일부가 아니다. 보존된 동작에 결함이 발견되면 먼저 재현 증거를 만들고 별도 사용자
결정을 받는다.
...
각 runtime slice는 독립 PR, CI, Railway 배포와 production smoke checkpoint다. 이전 slice의
종료조건이 통과하기 전에는 다음 slice를 시작하지 않는다.

### M4.6-A — 기준선과 규범 정렬

상태: active. runtime 동작은 바꾸지 않는다.
...
2. exact·bounded·relative-only Event는 근거가 허용한 y 범위 밖으로 나가지 않는다.
