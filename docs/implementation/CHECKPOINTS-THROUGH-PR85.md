# Historical implementation checkpoints through PR #85

Archived from CURRENT.md during IP-004 document cleanup. Historical context only;
see [CURRENT](CURRENT.md) for the active state. Revision and deployment statements
below describe their original checkpoints, not the latest runtime.
...
PR-0 baseline은 [evidence](../evidence/ip-004-pr0-baseline-2026-09-13.md)에 고정했다. PR-A1은 PR #81·runtime `54929f7`([evidence](../evidence/ip-004-pra1-island.md)), PR-A2는 PR #82·runtime `9456f13`으로 배포 검증했다([evidence](../evidence/ip-004-pra2-event.md)). PR-B1은 PR #83·runtime `a6e9ec6`의 Revision-safe readback과 실제 Clotho coarse commit을 통해 revision 2까지 검증했다([evidence](../evidence/ip-004-prb1-coarse.md)). 현재 PR-B2 — 점진적 detail·해석/불확실성 검증. PR-C는 이 실제 query shape를 계측한다. Graph renderer/layout/interaction은 M4.7 baseline을 유지한다.
...
| 실행 상태                  | `active` — PR-0, PR-A1/A2, PR-B1 실제 coarse readback 완료; PR-B2 진행 중 |
| 활성 milestone             | IP-004 Production Readiness Gate — M5 entry blocker |
| 현재 slice                 | PR-B2 실제 Clotho progressive refinement·ambiguity·identity 재사용 |
