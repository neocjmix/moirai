...
노량 직접 URL의 모바일 재검증에서는 공유 Event가 Canon별로 다른 layout x 좌표를 가질 때 단일 Canon bootstrap과 복수 Canon viewport의 대표 geometry가 달랐다. focus 조회도 실제 선택된 전체 source 집합으로 수행하여 동일한 dedup/World offset을 거치도록 수정한다. 서로 다른 Canon membership/관계로 실제 spatial artifact를 생성하는 회귀 검사는 원 구현에서 실패하고 수정본에서 통과한다. drawer 닫기 뒤 selected retention 없이도 공유 Event가 화면 안에 하나만 남아야 한다.
