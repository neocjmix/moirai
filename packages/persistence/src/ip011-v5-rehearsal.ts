...
export async function rehearseV5Database(
  ...
  try {
    ...
    const result = await applyReviewedV5Cutover(target, input);
    if (
...
/** Same reviewed transaction and readback for clone and controlled live cutover.
 * The caller owns fresh backup/restore, write quiescence and source identity. */
export async function applyReviewedV5Cutover(
  target: MoiraiDatabase,
  input: {
    readonly world_id: string;
    readonly expected_revision: number;
    readonly preservation_digest: string;
