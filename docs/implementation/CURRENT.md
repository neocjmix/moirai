...
현재 배포는 v4 Canon 모델이다. main `0f86148a8463378e8b60e551490116ab59a04660`와 Railway 세 서비스 latest SUCCESS가 일치한다. 역사 World Live export는 revision30이다. IP-004~010 완료 이력은 유지한다.
...
A2는 [owner-full DB inventory와 암호화 backup/restore 도구](../evidence/ip011/a2-execution.md)를 PR #131로 배포하고 실제 운영 snapshot의 암호화 저장·격리 복원 digest 일치를 확인했다. PR #132의 v4 history reader 분리도 CI 통과 후 병합·세 서비스 배포했다. 160→133 Narrative 보존 manifest의 offline 검증을 마쳤으며, 이제 v5 계약 구현과 복원본 전환 검증을 진행한다. 운영 schema/content migration은 아직 실행하지 않았다. 그다음 v5+데이터 rehearsal → A3 cutover → A4 bounded read → A5 discovery → A6 역사 dogfooding이다.
