...
> Superseded 2026-09-12: A~G의 완료 결과는 유지한다. H native renderer는 철회됐고,
> 이 문서에 남은 graph viewport 구현·종료 계획은 더 이상 실행하지 않는다. 후속 graph
> 작업은 [M4.6 — URDR graph pipeline 위의 Moirai data](M4.6-URDR-GRAPH-MOIRAI-DATA.md)가
> 대체한다.
...
M4.5-A~G는 완료 checkpoint다. H는 구현·검증 이력만 보존하고 철회됐다. M4.5의 남은
구현 계획과 전체 종료조건은 폐기됐으며 M4.6이 후속 graph 작업을 소유한다. M5는 M4.6이
끝나고 사용자가 전환을 승인할 때까지 비활성이다.
...
상태: complete. PR #42, merge `f414f571def404070a0ea7f4812d1809437b0860`에서
temporary bridge와 규모 기준을 검증했다. H에서 adapter와 loss-report 계약을 제거했다.
...
상태: withdrawn. [PR #50](https://github.com/neocjmix/moirai/pull/50), merge·production
`8d1fa038dee1d595607edfd0cfc2d52b6b0ac8d3`의 구현·검증 이력은 보존한다. 그러나
2026-09-12 사용자 판단으로 해당 native viewport와 layout은 runtime 기준에서 철회하고,
Publication query·검색 개선을 유지한 채 원본 URDR 목데이터 viewport로 복구했다.
후속 renderer 재설계는 별도 승인 전까지 시작하지 않는다.
...
이 종료조건은 역사 기록이며 더 이상 실행 기준이 아니다. M4.5-H 철회로 native renderer와
실제 Publication graph 표현 항목은 충족한 것으로 간주하지 않는다. M4.5-A~G의
데이터·query 결과는 유지하고 후속 작업은 M4.6의 종료조건을 따른다.
