...
> 완료됐고 Relation DP-001에서 `blocked_on_decision`이다. 아래 기존 D~H의 실행 단위는
> 그대로 재개하지 않는다. 승인 후 적용할 새 순서는
...
M4.5-C는 완료 checkpoint다. M4.5의 후속 실행은 IP-003 DP-001과 재설계 승인 대기 상태다.
