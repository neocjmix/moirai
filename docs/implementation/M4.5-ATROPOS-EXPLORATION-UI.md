...
> IP-003 interrupt 당시 M4.5-C는 완료 checkpoint로 보존했고 기존 M4.5-D는 폐기·분할했다.
> 이 문서의 `canon_id` 단일 source-address와
> Canon partition을 전제한 query/result 예시는 IP-003 이전 역사적 계약이며 신규 구현의
> source of truth가 아니다.

> 2026-09-11 재설계 checkpoint: IP-003 Event/Canon/R1 Relation cutover와 production acceptance를
> 완료했다. 기존 D~H를 그대로 재개하지 않고, 사용자 승인에 따라 새 순서
...
> D1~H는 모두 완료했고 M5 activation 전에서 정지한다.
...
M4.5-A~C와 재설계된 D1·D2·E·F·G·H는 완료 checkpoint다. M5는 사용자가 지정한
정지점에 따라 비활성이다.
...
apply/cancel을 거치며 incompatible World는 이유와 함께 비활성화된다. C 당시 source catalog는
실제 Publication composition 전 UI 계약을 검증하는 명시적 `MOCK` fixture였고 E에서 실제
Publication composition으로 교체했다. CI
...
상태: superseded. 이 결합 slice는 실행하지 않았다. IP-003 결과에 따라 D1과 D2로 분할했고
두 재설계 slice는 모두 complete다.
...
상태: complete. PR #42, CI `34617997999`, merge `f414f571def404070a0ea7f4812d1809437b0860`에서 완료했다.
...
상태: complete. PR #45와 production stabilization PR #46·#47, CI·Railway·post-deploy
검증으로 종료했다.
...
M4.5는 완료했다. M5는 사용자가 별도로 시작할 때까지 활성화하지 않는다.
