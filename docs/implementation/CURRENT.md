...
현재 배포는 v4 Canon 모델이다. main `c21d298d3987fb8d9266ad5bb0345a8ab000c9ec`와 Railway 세 서비스 latest SUCCESS가 일치한다. 역사 World의 마지막 확인된 Revision은 30이다. IP-004~010 완료 이력은 유지한다.
...
A2는 [owner-full DB inventory와 암호화 backup/restore 도구](../evidence/ip011/a2-execution.md)를 PR #131로 배포하고 실제 운영 snapshot의 암호화 저장·격리 복원 digest 일치를 확인했다. PR #132는 v4 history reader를 보존했다. PR #133은 160→133 Narrative 보존 manifest를 병합했고, PR #134는 v5 target 불변식·원자 candidate·격리 schema gate를 병합했다. 현재 실제 복원본에서 schema+content 전환을 시험하는 실행기와 CI를 진행 중이다. 운영 schema/content migration은 아직 실행하지 않았다. 그다음 A3 cutover → A4 bounded read → A5 discovery → A6 역사 dogfooding이다.
