...
M4.5-C와 재설계된 D1·D2는 완료 checkpoint다. 2026-09-11 사용자 승인으로 E~H를
순차 실행 중이다.
...
M4.5-H 철회로 위 전체 종료조건 중 native renderer와 실제 Publication graph 표현 항목은
더 이상 충족한 것으로 간주하지 않는다. M4.5-D1~G의 데이터·query 결과는 유지하며,
후속 renderer 재설계와 M5는 활성화하지 않는다.
