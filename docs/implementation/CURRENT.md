...
현재 공개 UI·writer·MCP/CLI는 여전히 v4다. v5 writer/transport는 내부 또는 별도 미등록 경계이고, 완료된 v5 Publication pointer는 없다. `/graph/v5/viewport`·`/graph/v5/read`는 complete root만 수용하므로 현재 staged root로는 503이다. 새로운 v5 탐색 페이지도 complete pointer가 있어야 진입할 수 있으며 공개 cutover가 아니다. 실제 clone Publication rehearsal은 Relation 선의 Event 중복을 고친 뒤 125개 배치·2개 미배치, 192페이지, 페이지당 최대 object read 14회로 통과했다. 이는 127-Event 규모의 staged read 검증이다. 다음 offline complete producer는 모든 Collection×Time System 선택 read의 정확성을 검증한 뒤에만 별도 complete index를 만들며, 아직 실제 clone에서 실행하지 않았고 pointer를 쓰지 않는다. pointer·UI E2E·external parity·owner-full history/import 증거와 큰 World의 scale/latency 검증이 없으므로 A2를 종료하지 않는다. 이후 A3 cutover → A4 bounded read → A5 discovery → A6 역사 dogfooding이다.
