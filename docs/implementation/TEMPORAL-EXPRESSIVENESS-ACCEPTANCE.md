...
[TS-010](../technical-specifications/TS-010-event-relational-time.md)의 의미 계약은 2026-09-05 승인됐다. 이 문서와 machine-readable fixture는 승인된 시험 계약이지만 runtime·schema·배포 또는 시험 World 쓰기를 자동 승인하지 않는다.
...
fixture는 결정된 하나의 Time System과 definition version을 사용한다. 수용시험 Time System은 `proleptic-gregorian-utc@1`이다. 좌표 문자열의 자릿수는 의미가 있으므로 숫자형으로 변환하지 않는다.

```text
time-system: proleptic-gregorian-utc
definition-version: 1
coordinate-format: YYYY-MM-DDTHH:mm:ss.ffffffffffffZ
capabilities: canonicalize, equality, compare, boundary, difference
boundary-policy: half-open [start, nextBoundary)
```
...
다음 세 의미 계약은 2026-09-05 승인됐다.

- `precedes`는 strict `<`, `not_after`는 `≤`, `coincides`는 대칭인 `=`다.
- virtual Time Event는 tagged structured reference이며 create·update·withdraw하지 않는다.
- 좌표는 Time System별 lossless canonical string이다. 수용시험 adapter는 UTC proleptic Gregorian, 12자리 fraction과 반개구간 경계를 사용한다.

다음 실행 계약은 각 구현 slice가 실제 관찰면을 제공하면서 닫는다.
...
Slice 0 fixture는 [`fixtures/temporal-expressiveness`](fixtures/temporal-expressiveness) 아래에 둔다. expected 파일은 구현 내부 snapshot이 아니라 제품 경로가 만족해야 할 의미 assertion이다.
...
| `te-exact-observation` | `정확한 순간에 기록된 관측`            | `event coincides T(2026-09-05T08:13:21.123456789012Z)`                                                                            | 일반 Event와 정확한 순간의 동시성                           |
...
`≤`는 lower-bound `not_after`, `<`는 strict `precedes`로 직렬화한다. exact same-instant는 `coincides`다.
...
| `te-exact-observation` | `coincides` 근거의 exact coordinate               | 범위로 확장하거나 별도 timestamp를 발명          |
...
| `te-bad-system`          | conversion이 없는데 cross-system coordinate comparison으로 파생했다고 주장 | 두 Time System과 필요한 conversion capability 표시 |
