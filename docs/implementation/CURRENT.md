...
A2의 격리 clone은 별도 [A2 gate review](../evidence/ip011/a2-gate-review.md)에 기록한다. 운영 공개 UI는 `/graph/v5`와 검증된 포인터를 통해 v5를 읽으며 공간 요약은 125개 배치·2개 미배치를 반환한다. 연결된 Moirai Live 도구 catalog는 아직 v4 입력을 광고하지만, 인증된 v5 commit/replay는 임시로 제한한 운영 API action의 성공 로그로 확인했다. 세부 시나리오별 통과와 미검증 사항은 A3 실행 기록을 따른다.
