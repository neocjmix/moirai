...
검증 기간: 2026-09-02~04. 계약: CON-004, TS-004, TS-008, IS-001, IP-001 M3-C.
...
사용자 결정에 따라 동일 기발급 토큰의 긴급 차단 live drill은 운영자 후속 검증으로 보류한다. [운영자 실행 가이드](M3-C-OIDC-DRILL.md)에 사전 조건, 정상→차단→복원 순서, 즉시 복구 조건, 성공 판정과 공개 가능한 증거 형식을 기록했다. 자동 fail-closed 테스트와 실제 metadata 차단·복원 근거는 유지하되 live HTTP 401을 확인한 것으로 표현하지 않는다.

실제 OAuth 읽기·validate 무저장·원자적 commit·replay·변경 replay 거절·Atropos 공개 반영·허용 World 밖 거절과 복구 후 재연결까지 확인했으므로 M3-C는 이 제한을 명시한 채 마감한다. 시험 앱은 후속 drill 재사용을 위해 남긴다. 이 마감은 M4를 활성화하지 않는다.
