...
현재 공개 UI·writer·MCP/CLI는 여전히 v4다. v5 writer/transport는 내부 또는 별도 미등록 경계이고, 운영 v5 Publication pointer는 없다. `/graph/v5/viewport`·`/graph/v5/read`와 v5 탐색 페이지는 실제 complete pointer만 수용하므로 여전히 비활성이다. 격리 clone의 Rev31 complete 후보는 125개 배치·2개 미배치, pointer/storage write 0으로 통과했다. #173 staged 측정(192페이지·최대 read 14회)과 #175 이후 재측정(48페이지·최대 read 38회)의 차이는 미해결이다. #177 배포의 clone에서 complete 후보를 두 번 재실행하여 같은 Revision·문서 집합/완성 root digest·48/38을 확인했지만 digest를 기록하지 않은 #173과의 차이 원인은 소급 입증하지 못했다. 모바일 CI는 한 자식과 좌표가 겹치는 Composite의 접근 문제를 수정했고 격리 브라우저 fixture에서 shared Event·Narrative·미배치·Composite 탐색을 통과했다. complete pointer handoff는 격리 단위 테스트를 통과했지만 실제 worker 호출은 금지한 상태다. authoring의 World 범위 후보 검색·상세 읽기, external parity와 owner-full history/import 경계의 잔여 검증이 있으므로 A2를 종료하지 않는다. 큰 World의 scale/latency 실측과 최적화는 A4 exit이며 A3 전에 필요한 구조·상한만 A2에서 확인한다. 이후 A3 cutover → A4 bounded read → A5 discovery → A6 역사 dogfooding이다.
