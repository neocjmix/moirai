...
정사 삼국지와 삼국지연의처럼 여러 Canon은 같은 인물과 시대를 다루면서 서로 다른 사건·관계·시간을 각각 사실로 가질 수 있다.
...
4. Event, Relation, Time System과 Narrative를 새 Canon의 사실과 표현으로 작성한다.
5. 작성자는 Canon의 경계와 내부 일관성, 다른 Canon과의 공통점과 차이를 검토한다.
6. 승인된 Canon은 기존 Canon과 우열 없이 같은 World 안에서 관리된다.
7. 출판자는 각 Canon에서 공개할 범위와 시점을 결정한다. 이 결정은 Canon의 진실 지위와 무관하다.
8. Atropos는 독자가 현재 보고 있는 Canon을 알 수 있게 하고, 공개된 Canon들의 공통점과 차이를 탐색할 수 있게 한다.
...
- 출판되었는지 여부와 더 많은 Source를 가졌는지는 Canon의 진실 지위를 바꾸지 않는다.
- Canon 사이에 공유되는 대상이 있어도 서로 다른 Event와 Relation을 임의로 합쳐서는 안 된다.
- Canon 비교는 차이를 보여줄 뿐 하나의 사실로 통합하거나 진위를 판정하지 않는다.
