...
> IP-003 interrupt: M4.5-C는 완료 checkpoint로 보존한다. M4.5-D~H는 IP-003과
> 후속 재설계의 사용자 승인 전까지 비활성이다. 이 문서의 `canon_id` 단일 source-address와
> Canon partition을 전제한 query/result 예시는 IP-003 이전 역사적 계약이며 신규 구현의
> source of truth가 아니다.

> 2026-09-11 재설계 checkpoint: IP-003 Event/Canon cutover와 production acceptance는
> 완료됐고 Relation DP-001은 R1으로 승인되어 IP-003 Slice 5가 진행 중이다. 아래 기존 D~H의 실행 단위는
> 그대로 재개하지 않는다. IP-003 완료 후 승인받을 새 순서는
...
M4.5-C는 완료 checkpoint다. M4.5의 후속 실행은 IP-003 완료와 재설계 승인 대기 상태다.
...
공개 schema ID는 `moirai.graph-query.v1`, `moirai.graph-query-result.v2`,
...
IP-003은 query와 URL state v1을 유지해 M4.5-C 복원을 보존하고 result만 v2로 올렸다.
v2 Event는 단일 `canon_id` 대신 `world_id`, 전체 `canon_memberships`와 선택 source에 실제로
...
### M4.5-D — Entities·Relations·Search·Diagnostics

상태: inactive. M4.5-C 완료 뒤 대기 중이며 사용자 전환 지시 전에는 시작하지 않는다.
...
- `MoiraiGraphQueryResult`에서 기존 viewport prop으로 가는 단방향 adapter를 구현한다.
